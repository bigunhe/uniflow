      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-fit rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-70"
          >
            {saving ? "Submitting..." : "Submit Request"}
          </button>
        </form>

        {message ? <p className="mt-3 rounded-lg bg-slate-100 p-2 text-sm text-slate-700">{message}</p> : null}
      </section>

      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900">Recommended Mentors</h2>
          <Link href="/student/urgent" className="text-sm font-semibold text-rose-700 hover:text-rose-800">
            Need immediate help?
          </Link>
        </div>
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {recommendedMentors.length === 0 ? (
            <p className="text-sm text-slate-500">Submit a request to see smart matches.</p>
          ) : (
            recommendedMentors.map((mentor) => (
              <article key={mentor.id} className="rounded-lg border border-slate-200 p-3">
