...
export const searchVectorItemSchema = z.object({
  query: z.string().min(8),
  sourceFiles: z.array(z.string()).min(1).max(4),
});

export const geminiModuleInsightBodySchema = z.object({
  coreModels: z.array(moduleInsightCoreSchema).min(1).max(6),
  searchVectors: z.array(searchVectorItemSchema).min(2).max(12),
...
export type SearchVectorItem = z.infer<typeof searchVectorItemSchema>;
...
export const storedModuleInsightsSchema = z.object({
  ...
  digestTruncated: z.boolean().optional(),
  digestFilesIncluded: z.array(z.string()).optional(),
...
export function parseStoredModuleInsightsJson(raw: string): StoredModuleInsights | null {
  try {
    ...
    // v1 cache invalidated — require "Generate AI insights" again
    return null;
  } catch {
    return null;
  }
...
export async function uploadStoredModuleInsights(
  ...
  const payload: StoredModuleInsights = {
    ...
    model: args.model,
    digestTruncated: args.digestTruncated,
    digestFilesIncluded: args.digestFilesIncluded,
